...
Publisher.prototype.publish = function (headers, options) {
  ...
  // init regex headers
  if (!options.headersRegex) options.headersRegex = {};
  ...
  return through.obj(function (file, enc, cb) {
    ...
    if (file.isBuffer()) {
      ...
      for (reg in options.headersRegex){
        // check regex filter
        if (file.s3.path.match(new RegExp(reg))){
          // add headers
          for (header in options.headersRegex[reg]) {
            file.s3.headers[header] = options.headersRegex[reg][header]
          }
        }
      }
